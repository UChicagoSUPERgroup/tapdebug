...
from autotapta.model.Rule import RegRule, SymbRule, ChangedRule
from autotapta.model.Util import EventExpression, ActionExpression, ConditionExpression, TriggerExpression, \
                                 Comparator, ComparatorRange, checkExpression, comp_range_dict, comp_dict
import datetime
...
class DebugSystem(ParameterizedSystem):
    """
    A symbolic rule system that tries to modify rules' conditions
    1) adding one condition to a rule
    2) deleting one condition in a rule
    2) modifying condition parameters in a rule
    """
    def __init__(self, cap_list: List[str], target_action: str, cond_cap_list: List[str], 
        ...
        for tap in tap_list:
            ...
            tap_condition = [self._formulaToExpression(cond, 'condition') for cond in tap.condition]
            tap_action = self._formulaToExpression(tap.action, 'action')
            rule = ChangedRule(tap_trigger, tap_condition, tap_action)
            self.existing_rule_list.append(rule)
        ...
        # initialize rule selection bits (this is used for adding condition/modifying condition)
        ...
        # initialize rule condition selection bits
        self.tap_cond_sel_list = [rule.cond_show_symbs for rule in self.existing_rule_list]
        for rule in self.existing_rule_list:
            self.z3_var_dict.update(rule.z3_var_dict)
            self.z3_var_default_val_dict.update(rule.z3_var_default_val_dict)

        # initialize new condition selection bits (the new condition is added to the rule selected by rule selection bit)
        self.cond_cap_list = cond_cap_list
    ...
    def applyAction(self, event_f):
        ...
        for cond, cond_show, cond_type, cond_comp, cond_val in zip(self.cond_cap_list, self.cond_show_list, 
            ...
            cond_sat_list.append(cond_sat)
        additional_cond = z3.And(cond_sat_list)
        # trigger all existing rules
        trigger_bits = []
        for rule, rule_sel in zip(self.existing_rule_list, self.tap_sel_list):
            # whether the rule is triggered
            action, trig_cond = rule.trigger(event, pre_cond_dict)
            # check if additional condition is triggered
            trigger_cond = z3.And(trig_cond, z3.Implies(rule_sel, additional_cond))
            trigger_bits.append(trigger_cond)
            if action is not None and trig_cond is not None:
                action_index = self.cap_list.index(action.var)
                self.value_list[action_index] = z3.If(trig_cond, action.val, self.value_list[action_index])
    ...
    def timePass(self, start_time: datetime.datetime, end_time: datetime.datetime):
        # TODO: timepass needs to be supported in ChangedRule and here
        pass
    ...
    def getSyntaxConstraints(self):
        ...
        1) when clause doesn't show up, use default value
        2) for boolean ap, keep using '=false' instead of '!=true'
        3) for range ap, shouldn't be using '=' or '!=', value should >= 0
        ...
        # 2) for boolean ap, keep using '=false' instead of '!=true' 
        #    (this is deleted. deleting cond/adding cond will handle this differently)
        ...
        for cond_typ, cond_val in zip(self.cond_cap_type_list, self.cond_val_list):
            if cond_typ == 'numeric':
                constraints.append(cond_val >= 0)
        # 4) there should only be one rule being changed (this is deleted. deleting cond/adding cond will handle this differently)
        # tap_sel_list_n = [z3.If(tap_sel, 1, 0) for tap_sel in self.tap_sel_list]
        # constraints.append(sum(tap_sel_list_n) == 1)
        return constraints

    def getConstraintsForDeleting(self, rule_cond_ids=None):
        """
        deleting one condition
        TODO: sometimes we can force which conditions to delete with rule_cond_ids
        :return:
        1) only one cond is deleted
        2) all aps not showing up
        3) no rule is selected
        """
        constraints = []
        # 1) only one cond is deleted
        cond_sel_sum_list = []
        for cond_sel_list in self.tap_cond_sel_list:
            cond_sel_sum = sum(z3.If(cond_sel, 0, 1) for cond_sel in cond_sel_list)
            cond_sel_sum_list.append(cond_sel_sum)
        constraints += [sum(cond_sel_sum_list) == 1]
        # 2) all aps not showing up
        constraints += [z3.Not(cond_show) for cond_show in self.cond_show_list]
        # 3) no rule is selected
        constraints += [z3.Not(tap_sel) for tap_sel in self.tap_sel_list]
        return constraints

    def getConstraintsForModifying(self, rule_cond_ids=None):
        """
        modifying one condition
        changing the condition of one rule
        TODO: sometimes we can force which conditions to modify with rule_cond_ids
        :return:
        1) only one cond is deleted
        2) only 1 new cond showing up
        3) var and comp should remain the same
        4) only one rule is selected (the same as the rule whose cond is deleted)
        """
        constraints = []
        # 1) only one cond is deleted
        cond_sel_sum_list = []
        ...
        constraints += [sum(cond_sel_sum_list) == 1]
        # 2) only 1 new cond showing up
        constraints += [sum(z3.If(cond_show, 1, 0) for cond_show in self.cond_show_list) == 1]
        # 3) var and comp should remain the same
        for cond_sel_list, rule in zip(self.tap_cond_sel_list, self.existing_rule_list):
            for cond_sel, cond in zip(cond_sel_list, rule.condition):
                # var and comp should remain the same if cond is selected
                # val should be different
                cond_index = self.cond_cap_list.index(cond.var)
                cond_show = self.cond_show_list[cond_index]  # this should be true if cond is deleted
                cap_same = cond_show
                cond_comp = self.cond_comp_list[cond_index]
                cond_type = self.cond_cap_type_list[cond_index]
                comp_same = cond_comp == comp_range_dict[cond.comp] if cond_type == 'numeric' else cond_comp == comp_dict[cond.comp]
                cond_val = self.cond_val_list[cond_index]
                cond_diff = cond_val != cond.val
                constraints.append(z3.Implies(z3.Not(cond_sel), z3.And([cap_same, comp_same, cond_diff])))
        # 4) only one rule is selected (the same as the rule whose cond is deleted)
        for tap_sel, cond_sel_list in zip(self.tap_sel_list, self.tap_cond_sel_list):
            constraints += [tap_sel == z3.Not(z3.And([cond_sel for cond_sel in cond_sel_list]))]
        return constraints
    
    def getConstraintsForAdding(self, rule_ids=None):
        """
        adding one condition
        changing the condition of one rule
        :return:
        1) no cond is deleted
        2) only 1 new cond showing up
        3) only one rule is selected
        4) for boolean ap, keep using '=false' instead of '!=true'
        5) for range ap, shouldn't be using '=', '!=', '<=' or '>='
        """
        constraints = []
        # 1) no cond is deleted
        for cond_sel_list in self.tap_cond_sel_list:
            constraints += [cond_sel for cond_sel in cond_sel_list]
        # 2) only 1 new cond showing up
        constraints += [sum(z3.If(cond_show, 1, 0) for cond_show in self.cond_show_list) == 1]
        # 3) only one rule is selected
        constraints += [sum(z3.If(tap_sel, 1, 0) for tap_sel in self.tap_sel_list) == 1]
        # 4) for boolean ap, keep using '=false' instead of '!=true'
        for cond_show, cond_typ, cond_comp in zip(self.cond_show_list, self.cond_cap_type_list, self.cond_comp_list):
           if cond_typ == 'bool':
               constraints.append(z3.Implies(cond_show, cond_comp == Comparator.eq))
            ...
            if cond_typ == 'numeric':
                constraints.append(z3.Implies(cond_show, 
                                              z3.And(cond_comp != ComparatorRange.eq, 
                                                     ...
                                                     cond_comp != ComparatorRange.geq,
                                                     cond_comp != ComparatorRange.leq)))
        # if the user specifies some rules to while we want to add conditions, force that
        if rule_ids is not None:
            tap_sels = [self.tap_sel_list[i] for i in rule_ids]
            constraints.append(z3.Or(tap_sels))
        return constraints
    ...
    def generatePatchDescription(self, z3_eval_dict):
        add_cond_rule_id = None  # the rule to which we add contidion
        rule_condition_id = None  # the rule one of whose condition is deleted
        condition_id = None  # the condition id to be deleted within rule
        new_condition = None  # the new condition added

        for rule_sel, index in zip(self.tap_sel_list, range(len(self.tap_sel_list))):
            if z3_eval_dict[str(rule_sel)]:
                add_cond_rule_id = index
                break
        
        for cond_show_list, index in zip(self.tap_cond_sel_list, range(len(self.tap_cond_sel_list))):
            for cond_show, cond_index in zip(cond_show_list, range(len(cond_show_list))):
                if not z3_eval_dict[str(cond_show)]:
                    # the condition is deleted
                    rule_condition_id = index
                    condition_id = cond_index
                    break
            if condition_id is not None:
                break
        
        new_condition = self.getNewConditionInFormula(z3_eval_dict)
        
        desc = {}
        if add_cond_rule_id is None:
            # should be deleting condition
            desc['mode'] = 'delete-condition'
            desc['rule_id'] = rule_condition_id
            desc['condition_id'] = condition_id
            desc['new_condition'] = ''
        else:
            # should be adding or modifying
            if condition_id is None:
                # adding
                desc['mode'] = 'add-condition'
                desc['rule_id'] = add_cond_rule_id
                desc['condition_id'] = -1
                desc['new_condition'] = new_condition
            else:
                # modifying
                assert(rule_condition_id == add_cond_rule_id)
                desc['mode'] = 'modify-condition'
                desc['rule_id'] = add_cond_rule_id
                desc['condition_id'] = condition_id
                desc['new_condition'] = new_condition

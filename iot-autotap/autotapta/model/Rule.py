...
class SymbRule(Rule):
    ...
    def timePass(self, start_time: datetime.datetime, end_time: datetime.datetime, pre_condition_dict: Dict[str, Any]):
        ...
        clock_sat = z3.And(z3.Or(clock_sat_list), self.clock_trig_show)
        cond_sat_list = [z3.Implies(ap_show, checkExpression(pre_condition_dict[ap_cap], ap_val, ap_comp, ap_typ))
                         for ap_cap, ap_typ, ap_show, ap_comp, ap_val in
                         zip(self.ap_cap_list, self.ap_cap_typ_list, self.ap_show_list,
                             self.ap_comp_list, self.ap_val_list)]
    ...
    def getSyntaxConstraints(self):
        ...
        4) for range ap, shouldn't be using '=' or '!=', and >= 0
        ...
        for trig_show, trig_comp, trig_typ, trig_val in zip(self.trig_show_list, self.trig_comp_list, self.trig_cap_typ_list, self.trig_val_list):
            if trig_typ == 'numeric':
                ...
                constraints.append(trig_val >= 0)
        for timing_trig_show, timing_trig_comp, timing_trig_typ, timing_trig_val in zip(self.timing_trig_show_list,
                                                                                        self.timing_trig_comp_list,
            ...
            if timing_trig_typ == 'numeric':
                ...
                constraints.append(timing_trig_val >= 0)
        for ap_show, ap_comp, ap_typ, ap_val in zip(self.ap_show_list, self.ap_comp_list, self.ap_cap_typ_list, self.ap_val_list):
            if ap_typ == 'numeric':
...
class ChangedRule(Rule):
    # a rule whose conditions might change
    # each condition has a symbol to indicate whether it shows up
    def __init__(self, trigger: TriggerExpression, condition: List[ConditionExpression], action: ActionExpression):
        # trigger, condition and action
        self.trigger_e = trigger
        self.condition = condition
        self.action = action

        self.z3_var_default_val_dict = {}
        self.z3_var_dict = {}

        # showing symbol for each condition
        self.cond_show_symbs = [z3.Bool('origcondshow_%d_%d' % (id(self), index)) for index in range(len(condition))]
        for cond_symb in self.cond_show_symbs:
            self.z3_var_dict[str(cond_symb)] = cond_symb
            self.z3_var_default_val_dict[str(cond_symb)] = True

    def trigger(self, event: EventExpression, pre_condition_dict: Dict[str, Any]):
        cond_sat = z3.And([
            z3.Implies(show_symb, cond.check(cond.var, pre_condition_dict[cond.var])) 
            for cond, show_symb in zip(self.condition, self.cond_show_symbs)])
        if self.trigger_e.var != 'clock.clock_time' and self.trigger_e.isRegular():
            cond_sat = z3.And(cond_sat, z3.Not(self.trigger_e.checkTrigger(pre_condition_dict[self.trigger_e.var])))
    ...
    def timePass(self, start_time: datetime.datetime, end_time: datetime.datetime, pre_condition_dict: Dict[str, Any]):
        ...
        return self.action, z3.And(self.trigger_e.checkSpan(start_time, end_time), cond_sat)

    def to_tap(self, var_resolve: dict={}):
        if any(str(show_symb) not in var_resolve for show_symb in self.cond_show_symbs):
            raise Exception('cannot translate symbolic rule into real tap without assignment')
        ...
        if self.trigger_e.hold_t:
            ...
            trigger = '%s%s%s' % (self.trigger_e.var, self.trigger_e.comp, self.trigger_e.val)
        condition = ['%s%s%s' % (cond.var, cond.comp, cond.val) 
                     for cond, show_symb in zip(self.condition, self.cond_show_symbs) 
                     if var_resolve[str(show_symb)]]

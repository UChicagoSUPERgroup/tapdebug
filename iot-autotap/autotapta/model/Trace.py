...
class Trace(object):
    ...
    def addAction(self, time, action, is_ext=True):
        if not self.actions or time >= self.actions[-1][0]:
            ...
            if '#' in action or '*' in action or action.startswith('clock.clock_time') or \
               not self.pre_condition or not self.system.conditionSatisfied(action):
...
# return the times the event "clock.clock_time=clock_in_secs" happens
# within the time range [start_time, end_time)
# return all times when such event happens in a list of (time, event)
def _countClockEvents(start_time: datetime.datetime, end_time: datetime.datetime, clock_list):
    start_date = start_time.date()
    start_date_zero = datetime.datetime.combine(start_date, datetime.time())
    start_delta = start_time - start_date_zero
    end_delta = end_time - start_date_zero

    result_set = set()
    for clock_in_secs in clock_list:
        target_delta = datetime.timedelta(seconds=clock_in_secs)
        while start_delta <= target_delta < end_delta:
            result_set.add((start_date_zero + target_delta, 'clock.clock_time=%d' % clock_in_secs))
            target_delta += datetime.timedelta(days=1)

    return sorted(list(result_set))


# insert clock information into trace
# the clock_list consists of a list of time in seconds (xxxx)
# we can translate them into format xx:xx
# at the corresponding time in the trace, we add an event "clock.clock_time=xxxx"
def enhanceTraceWithClock(trace: Trace, clock_list: list):
    new_actions = list()
    new_pre_condition = list()
    new_is_ext = list()
    ...
    for time_action, pre_condition, is_ext in zip(trace.actions, trace.pre_condition, trace.is_ext_list):
        ...
        # first add the clock actions
        clock_events_list = _countClockEvents(current_time, time, clock_list)
        for t_e in clock_events_list:
            new_actions.append(t_e)
            new_pre_condition.append(pre_condition)
            new_is_ext.append(True)

        # then add the original action
        new_actions.append(time_action)
        new_pre_condition.append(pre_condition)
        new_is_ext.append(is_ext)
    ...
    # check if there are events before end_time
    clock_events_list = _countClockEvents(current_time, trace.end_time, clock_list)
    for t_e in clock_events_list:
        new_actions.append(t_e)
        new_pre_condition.append(pre_condition)
        new_is_ext.append(True)

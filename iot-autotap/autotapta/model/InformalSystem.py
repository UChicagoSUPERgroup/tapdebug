...
class InformalSystem(object):
    ...
    def _checkCurrentCondition(self, cond: str, time=None):
        if cond.startswith('!'):
            cond = cond[1:]
        if '<=' in cond:
            cap = cond.split('<=')[0]
        ...
        if cap == 'clock.clock_time':
            curr_value = time.hour * 3600 + time.minute * 60 + time.second
        else:
    ...
    def tapConditionSatisfied(self, tap: Tap, field=None, time=None):
        if field is not None:
            self.restoreFromStateVector(field)
        return all([self._checkCurrentCondition(cond, time=time) for cond in tap.condition])

    def conditionSatisfied(self, cond: str, field=None, time=None):
        if field:
            self.restoreFromStateVector(field)
        return self._checkCurrentCondition(cond, time=time)

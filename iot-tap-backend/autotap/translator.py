...
from backend.translator import trigger_to_clause, state_to_clause, time_to_int, action_to_clause
...
def autotap_simple_formula_to_clause(formula, flag=0, location=None, use_label=False, skip_channel=False):
    ...
    clause['capability'] = {'id': capability.id, 'name': capability.name, 'label': label, 'timelabel': capability.timeeventlabel}
    ...
    if formula.startswith('tick'):
        ...
        if '*' in formula:
            ...
            clause = {
                ...
                'capability': {'id': capability.id, 'name': capability.name, 'label': capability.eventlabel, 'timelabel': capability.timeeventlabel},
        ...
        clause = {
            'device': {'id': device.id, 'name': device.name},
            'capability': {'id': capability.id, 'name': capability.name, 'label': capability.statelabel, 'timelabel': capability.timeeventlabel},
            'channel': {'id': channel.id, 'name': channel.name, 'icon': channel.icon},
            'parameters': parameters,
            'parameterVals': parameter_vals}
        return _update_clause_text(clause)
...
def autotapta_formula_to_clause(formula, flag=0, location=None, use_label=True, skip_channel=True):
    ...
    if '*' in formula and flag == 1:
        # it becomes true that negative was last in effect more than time ago
        # with the new version as of tapdebug, we only add a timeValue field in the clause
        # instead of using the history clause
        ...
        statement_clause['timeValue'] = seconds_to_time(int(time))
        # statement_clause = _negate_clause(statement_clause)
        if location:
...
def patch_to_frontend_view(patch, location):
    # translate a patch (as dictionary) to format that frontend can read
    if patch['mode'] == 'add-rule':
        new_tap = patch['new_rule']
        clause = tap_to_frontend_view_ta(new_tap, location)
        patch['new_rule'] = clause
    elif patch['mode'] == 'delete-rule':
        pass
    elif patch['mode'] == 'add-condition':
        new_condition = patch['new_condition']
        clause = autotapta_formula_to_clause(new_condition, 0, use_label=True, location=location)
        patch['new_condition'] = clause
        ...
        clause = autotapta_formula_to_clause(new_condition, 0, use_label=True, location=location)
        patch['new_condition'] = clause
    elif patch['mode'] == 'modify-trigger':
        pass
    else:
...
def trigger_to_autotap_statement(clause, if_event=False):
    ...
    if 'timeValue' in clause and clause['timeValue'] is not None:
        # a timed event
        time_value = clause['timeValue']
        time_in_seconds = time_to_int(time_value)
        if time_in_seconds:
            clause_statement = '%d*%s' % (time_in_seconds, clause_statement)
    return clause_statement
...
# calculate the text for command (TapDebug proj)
# a command is a dict
# command
#   - capability
#      - label
...
#      - type, name, values
#   - value
def get_text_for_command(command, device):
    clause = {
        'parameters': [command['parameter']],
        'parameterVals': [{'value': command['value'], 'comparator': '='}],

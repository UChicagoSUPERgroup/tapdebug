...
export class ParameterConfigurationComponent {
  ...
  public timeLabelText: string[];
  public timeLabelSelectors: any[];
  public timeEventValue: any = {"hours":0,"minutes":0,"seconds":0};
  public rangeComparators: String[];
  public setComparators: String[];
  public color: string = "#000000";

  constructor(public userDataService: UserDataService, private cpService: ColorPickerService,
                        ...
                        .subscribe(
                          data => { 
                            ...
                            for (i = 0; i < this.parameters.length; i++) {
                              ...
                              } else if (this.parameters[i].type == "color") {
                                this.parameterVals.push({"value": "0,0", "comparator": "="});
                              } else {
                                this.parameterVals.push({"value": "", "comparator":"="});
                              }
                            }
  ...
  extractLabel(parameters: Parameter[]) {
    ...
    const timelabel = this.selectedCapability.timelabel;

    [this.labelText, this.labelSelectors] = this.extractLabelHelper(label, parameters);
    [this.timeLabelText, this.timeLabelSelectors] = this.extractLabelHelper(timelabel, parameters);
  }

  extractLabelHelper(label: string, parameters: Parameter[]) {
    let labelText = [];
    let labelSelectors = [];
        ...
        while (gettingParamName) {
          if (!['/', '}'].includes(label.charAt(i+j))) {
            ...
            if (paramName == "DEVICE") {
              ...
              gettingParam = false;
              text += (this.selectedDevice.label ? this.selectedDevice.label : this.selectedDevice.name) + " ";
              paramName = '';
              i += j;
              ...
              labelText.push(text);
              text='';
              labelSelectors.push([paramName, 'value', paramType, paramValPos])
              paramName = '';
              i += j;
            } else {
              gettingParamName = false;
            ...
            labelSelectors.push([paramName, choice, values, paramValPos]);
            paramName = '';
            i += jump-2;
          }
          j++;
    ...
    return [labelText, labelSelectors];
  }
  ...
  completeParameterSelect() {
    this.completeParameterSelectHelper(false);
  }

  completeParameterSelectWithTime() {
    this.completeParameterSelectHelper(true);
  }

  completeParameterSelectHelper(hasTime: boolean) {
    var valid = true;
    var i: number;
    // translate
    for (i = 0; i < this.parameterVals.length; i++) {
      if (this.parameters[i].type == 'color') {
        const hsva = this.cpService.stringToHsva(this.color, true);
        const hue: number = Math.floor(hsva.h * 100);
        const saturation: number = Math.floor(hsva.s * 100);
        this.parameterVals[i].value = String(hue)+','+String(saturation);
      }
    }
    ...
    // validation for time
    if (hasTime) {
      if (this.timeEventValue.hours == 0 && this.timeEventValue.minutes == 0 && this.timeEventValue.seconds == 0) {
        valid = false;
        invalidText += "parameter " + this.parameterVals.length + ": " + "time, ";
      }
    }

    // setup alert text for invalid data
    ...
    if (valid == false) {
      ...
      this.userDataService.currentClause.timeValue = hasTime ? this.timeEventValue: null;
      if (this.userDataService.historyClause) {

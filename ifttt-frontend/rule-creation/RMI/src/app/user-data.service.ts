...
export class UserDataService {
  ...
  // for showing visualization in admin page
  public visEntities: DevCapPar[];
  public startDatetime: string;
  public endDatetime: string;

  // zone name map
  // this is an ad-hoc way to store it
  // it should better be stored on the database
  // and communicated through the backend
  public zoneMap = {
    "A": "Living room",
    "B": "Bathroom #1",
    "C": "Device room",
  ...
  public selectHistoryClause(historyClauseParameterIndex: number, currentParameterVals: any[], temporality: string) {
    ...
    this.gotoDeviceSelector(this.selectedZone);
  }
  ...
  // navigates to zone selector for rules
  public gotoZoneSelector(objectType: string, index: number): void {
    if (objectType == 'trigger' || objectType == 'action') this.currentObjectType = objectType;
    if (objectType == 'triggerAdd') {this.currentObjectType = 'trigger';}
    this.currentObjectIndex = index;
    if (objectType == "trigger" && index <= 0) {
    ...
    this.router.navigate(['../create/selectZone'], {relativeTo: this.route})
  }

  // navigates to device selector for rules
  public gotoDeviceSelector(zone: Zone): void {
    this.selectedZone = zone;
    this.router.navigate(['../create/selectDevice', String(zone.id)], {relativeTo: this.route})
  }
  ...
  public getRules(): any {
      return this.http.get(this.apiUrl + "user/loc/" + "rules/");
  }
  ...
  // get all zones from the backend
  public getZones(): any {
    let body = {};
    let option = {headers: new HttpHeaders().set('Content-Type', 'application/json')};
    return this.http.post(this.apiUrl + "user/zones/", body, option);
  }
  ...
  public getDevicesInLocation(is_Trigger: boolean, is_Event: boolean, zone_id: number): any {
    let body = {};
    body = {"is_trigger": is_Trigger, "loc_id": this.current_loc, "zone_id": zone_id};
    let option = {headers: new HttpHeaders().set('Content-Type', 'application/json')};
  ...
  public getTextFromParVal(device: Device, capability: Capability, parameters: Parameter[], parameterVals: any[], hideDev: boolean = false, withTime: boolean = false): string {
    var label = withTime ? capability.timelabel: capability.label;
    if (hideDev) {
      label = label.replace("{DEVICE}'s", "");
      label = label.replace("({DEVICE})", "");
      label = label.replace("{DEVICE}", "");
    } else {
      label = label.replace("{DEVICE}", device.name);
    }
    var parameter: any;
  ...
  public getTextForClause(clause: Clause): Clause {
    // if timeValue is not null, use timed label. Use regular label otherwise
    clause.text = this.getTextFromParVal(clause.device, clause.capability, clause.parameters, clause.parameterVals, false, !!clause.timeValue);
    if (clause.timeValue)
      clause.text = this.updateTextForTiming(clause.text, clause.timeValue);
    return clause;
  }

  public updateTextForTiming(text: string, timeValue: any): string {
    text += " for exactly";
    if (timeValue.hours)
      text += " " + timeValue.hours + (timeValue.hours == 1 ? " hour": " hours");
    if (timeValue.minutes)
      text += " " + timeValue.minutes + (timeValue.hours == 1 ? " minute": " minutes");
    if (timeValue.seconds)
      text += " " + timeValue.seconds + (timeValue.seconds == 1 ? " second": " seconds");
    return text;
  }

  public getTextForCommand(device: Device, command: Command, hideDev: boolean=true): string {
    return this.getTextFromParVal(device, command.capability, [command.parameter], [{"value": command.value, "comparator": "="}], hideDev);
  }
  ...
  public getDefaultTextForCapability(capability: Capability, parameter: Parameter) {
    let device: Device = {
      ...
      mainStateLabel: null, subscribed: false, zone: {id: 0, name: ""}
    };
  ...
  public getDevices(locId: number, plain_revert: boolean=false, get_commands: boolean=true): any {
    let body = {"locid": locId, "plain_revert": plain_revert, "get_commands": get_commands};
    let option = {headers: new HttpHeaders().set('Content-Type', 'application/json')};
  ...
  public getRuleSuggestion(device: Device, command: Command) {
    let body = {"device": device, "command": command, "first_time": true, "locid": this.current_loc};
    let option = {headers: new HttpHeaders().set('Content-Type', 'application/json')};
  ...
  public fetchLog(token: string) {
    let body = {"token": token, "locid": this.current_loc};
    let option = {headers: new HttpHeaders().set('Content-Type', 'application/json')};
  ...
  // Manually get trace
  public fetchLogManual(locId: number, entities: DevCapPar[], startDatetime: string, endDatetime: string)
  {
    let body = {"locid": locId, "entities": entities, "start_time": startDatetime, "end_time": endDatetime};
    let option = {headers: new HttpHeaders().set('Content-Type', 'application/json')};
    return this.http.post(this.apiUrl + "autotap/getepisodemanual/", body, option);
  }

  // Get information about monitored trace
  public getMonitoredDevStatus(locId: number)
  {
    let body = {"loc_id": locId};
    let option = {headers: new HttpHeaders().set('Content-Type', 'application/json')};
    return this.http.post(this.apiUrl + "user/getmonitoreddevstatus/", body, option);
  }
  ...
  public getLocationToken() {
    /*  Return:
        200:  {"locid": "", "token": ""}
        401:  {"msg": "Please login."}
        500:  {"msg": repr(err)}
    */
    return this.http.get(this.apiUrl + 'user/get_loc_token/');
  }
  ...
  public parseRules(unparsedRules: Rule[]) {
    let RULES = unparsedRules.map((rule => {
      const words = [];
      const descriptions = [];
      const icons = [];
      const enhancedDesc = [];
      ...
      // let rule_ifClause = this.updateClausesTimezone(rule.ifClause, true);
      // console.log(rule_ifClause);
      for (let i = 0; i < rule.ifClause.length; i++) {
        ...
        enhancedDesc.push(this.parseLocationForRuleDescription(description));
      }

      // add the then clause
      words.push("then");
      // rule.thenClause[0] = this.getTextForClause(rule.thenClause[0]);
  ...
  parseLocationForRuleDescription(description: string) {
    let res = [];
    let re = /[A-Z] - /;
    ...
    if (!match) {
      ...
      if (index > 0) {
        res.push(["", description.slice(0, index)]);
      }
      res.push(["location", this.zoneMap[description.slice(index, index+1)]]);
      if (index + 4 < description.length) {
        res.push(["", description.slice(index+4)]);
      }
    }
    return res;
  }
